...
if ($stmt_images === false) {
    die('Error preparing query to get image: ' . $conn->error);
}
...
<head>
    ...
    <title>Tour Detail: <?php echo htmlspecialchars($tour['Tour_Name']); ?></title>
...
$isLoggedIn = isset($_SESSION['Traveller_Name']);
?>
    <div class="header">
    ...
    <div class="tour-details-container">
        ...
        <div class="tour-container">
            <div class="tour-name">
                <?php echo isset($tour['Tour_Name']) ? htmlspecialchars($tour['Tour_Name']) : 'No tour name'; ?>
            </div>
        ...
        ?>
            ...
            <div class="booking-section">
                ...
                <form action="process_booking.php" method="POST">
                    <label for="ticketQuantity">Quantity of ticket: </label>
                    <input type="number" id="ticketQuantity" name="ticketQuantity" min="1" value="1"
                        onchange="calculateTotal()">

                    <div>Price: <strong id="ticketPrice"><?php echo number_format($tour['Price']); ?> $</strong></div>
                    <div>Total price: <strong id="totalPrice"><?php echo number_format($tour['Price']); ?> $</strong>
                    </div>
...
// Hiển thị các phản hồi đã gửi trước đó
$query = "SELECT * FROM feedback WHERE TourID = ? ORDER BY created_at DESC";  // Loại bỏ Traveller_ID khỏi điều kiện
$stmt = mysqli_prepare($conn, $query);
mysqli_stmt_bind_param($stmt, 'i', $tour_id);  // Chỉ truyền TourID
...
?>
                ...
                <div class="container mt-5">
                    <h2 class="mb-4">Tour Feedback</h2>

                    <!-- Hiển thị tên người dùng đã đăng nhập -->
                    <p>Hello, <?php echo htmlspecialchars($_SESSION['Traveller_Name']); ?>!</p>
...
// Check login status for the traveller
$travellerName = isset($_SESSION['Traveller_Name']) ? $_SESSION['Traveller_Name'] : null;
$travellerId = isset($_SESSION['Traveller_ID']) ? intval($_SESSION['Traveller_ID']) : null;
$tourId = isset($_GET['TourID']) ? intval($_GET['TourID']) : (isset($_SESSION['Tour_ID']) ? intval($_SESSION['Tour_ID']) : null);

if (!$travellerId || !$tourId) {
    echo "Missing traveller or tour information!";
    exit;
}

// Fetch messages for the logged-in traveller
...
$stmtMessages->bind_param("iii", $tourId, $travellerId, $travellerId); // Use travellerId for both traveller and tourguide
    ...
    <div id="chat-box" style="display: none;">
        ...
        <div id="messages">
            ...
            <div
                ...
                <strong>
                    <?php 
                        // Display name based on message type (tourguide or traveller)
...
document.getElementById('send-btn')?.addEventListener('click', function() {
    ...
    if (message) {
        ...
        messageDiv.classList.add('message', 'traveller-message'); // Traveller sends the message
        messageDiv.innerHTML =
            `<strong><?php echo htmlspecialchars($travellerName); ?>:</strong> ${message} <span class="date-chat">Just now</span>`;
...
function saveMessageToDatabase(message) {
    const travellerId = <?php echo json_encode($travellerId); ?>;
    ...
    fetch('save_chat.php', {
            ...
            body: JSON.stringify({
                ...
                tourId,
                content: message,
                type: 'traveller' // Set the type to 'traveller' for traveller messages
            })
        })
